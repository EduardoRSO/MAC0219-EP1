...
// compute mandelbrot for each line in image
void * compute_mandelbrot_for_line(void * args) {
    int * line = (int *) args;
    int i_y = line[0];
	...
	for (iteration = 0;
	     ...
	     iteration++){
	    double temp_z_x = (z_x_squared) - (z_y_squared) + c_x;
	    double temp_z_y = (2 * z_x * z_y) + c_y;
...
// compute mandelbrot for each line as a thread
void compute_mandelbrot(){
    pthread_t * threads = (pthread_t *) malloc(sizeof(pthread_t) * i_y_max);
    int rc;
    int i_y;
	...
	rc = pthread_create(&threads[i_y], NULL, compute_mandelbrot_for_line, (void *) args);
	if(rc){
    ...
    for (i_y = 0; i_y < i_y_max; i_y++){
	rc = pthread_join(threads[i_y], NULL);
    	if(rc){
